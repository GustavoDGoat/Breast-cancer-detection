...
def main():
    def load_model():
        ...
        model.compile(
            optimizer=tf.keras.optimizers.legacy.Adam(
                learning_rate=0.00001, decay=0.0001
            ),
    ...
    )  # allows users to upload an image. The image is then converted to a NumPy array for processing.
    label = gr.Label(
        num_top_classes=8
    )  # shows the predicted class labels with confidence scores.
    ...
    """
    gr.Interface(fn=predict_img, inputs=image, outputs=label).launch(
        debug=True, share=True
    )
    """
    # Launch the Gradio interface
    # share=False avoids the network error you saw earlier
    gr.Interface(
        fn=predict_img, inputs=image, outputs=label, title="Oncology Breast Scanner"
